...
namespace GameModeLoader.Component {
	...
	public class SuperHot : LevelModule {
		...
		public override IEnumerator OnLoadCoroutine() {
			if (Level.current.GetOptionAsBool("superhot", true)) {
				...
				if ( WaveSpawner.instances.Count > 0 ) {
					...
					waveSpawner.OnWaveEndEvent.AddListener(new UnityAction(OnWaveEnded));
				}
			}
		...
		protected void OnWaveEnded() => this.waveEndedCoroutine = this.level.StartCoroutine(this.WaveEndedCoroutine());
		...
		public override void OnUnload() {
			if (Level.current.GetOptionAsBool("superhot", true)) {
				...
				enable = false;
				if (this.waveSpawner) {
					this.waveSpawner.OnWaveEndEvent.RemoveListener(new UnityAction(this.OnWaveEnded));
				}
			}
		...
		private float GetPlayerInput() {
			...
			var hands = (PlayerControl.handLeft.GetHandVelocity() + PlayerControl.handRight.GetHandVelocity()).magnitude / 2f;
