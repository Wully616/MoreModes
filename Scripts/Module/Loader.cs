...
namespace Wully.MoreModes
{
    public class Loader : CustomData
    {
            ...
            LevelOptionCatalog option)
        {
            ...
            if (gameMode.gameModeOptionIds != null && gameMode.gameModeOptionIds.Contains(option.id, StringComparer.OrdinalIgnoreCase))
            {
        ...
        private LevelDataModeCatalog AddOptionToGameMode(LevelDataModeCatalog gameMode, LevelOptionCatalog option)
        {
            ...
            if (option.excludeGameModeNames != null && option.excludeGameModeNames.Contains(gameMode.mode.name, StringComparer.OrdinalIgnoreCase))
            {
                return gameMode;
            }
        ...
        private void AddOptionsToLevelDataModes(LevelData levelData, List<LevelOptionCatalog> options)
        {
            foreach (var mode in levelData.modes)
            {
                foreach (LevelOptionCatalog option in options)
                {
                    ...
                    if ((option.excludeLevelIds == null || !option.excludeLevelIds.Contains(levelData.id, StringComparer.OrdinalIgnoreCase)
                        )
                        && (option.excludeGameModeNames == null || !option.excludeGameModeNames.Contains(mode.name)))
                    {
